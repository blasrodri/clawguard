...
export function runSetup(opts: { dryRun?: boolean; json?: boolean }): number {
...
function setup(dryRun: boolean): SetupResult {
  ...
  if (existsSync(pairedPath)) {
    ...
    for (const [id, raw] of Object.entries(paired)) {
      ...
      const scopes: string[] = Array.isArray(dev.scopes) ? (dev.scopes as string[]) : [];
      const approved: string[] = Array.isArray(dev.approvedScopes) ? (dev.approvedScopes as string[]) : [];
      ...
      if (!dryRun) {
        ...
        const tokens = dev.tokens as Record<string, unknown> | undefined;
        if (tokens) {
          for (const tok of Object.values(tokens)) {
            const t = tok as Record<string, unknown>;
            if (Array.isArray(t.scopes)) {
              t.scopes = [...new Set([...(t.scopes as string[]), ...needed])];
            }
          }
        }
        paired[id] = dev;
