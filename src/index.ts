...
export default definePluginEntry({
  ...
  api.on(HOOKS.beforeModelResolve, (ctx) =>
    guard("before_model_resolve", undefined, () => {
  ...
  api.on(HOOKS.beforeAgentRun, (ctx) =>
    guard("before_agent_run", config.failMode === "closed" ? { block: true } : undefined, () => {
      ...
      if (cancel) return { block: true, reason: `DLP: inbound message blocked (${labels.join(", ")})` };
      return undefined;
    }),
  );
  ...
  api.on(HOOKS.llmOutput, (ctx) =>
    guard("llm_output", undefined, () => {
      llmOutputFired = true;
  ...
  api.on(HOOKS.modelCallEnded, (ctx) =>
    guard("model_call_ended", undefined, () => {
...
function makeLogger(api: PluginApi): Logger {
  const l = api.logger;
  return {
    info: (msg: string) => (l?.info ? l.info(msg) : console.log(msg)),
    warn: (msg: string) => (l?.warn ? l.warn(msg) : console.warn(msg)),
  };
}
